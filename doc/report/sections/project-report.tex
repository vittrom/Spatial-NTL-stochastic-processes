...
\end{equation*}
Defining the cumulative birth measure as in \Cref{birth_measure} lets us relate random jumps in the Levy process $Z$ to random jumps in $B$ in exactly the same way as done in \cite{james2006poisson} for $Z$ and $\Lambda$. 
...
\end{equation}
of \Cref{joint_dist} where the time subscript of $F$ corresponds to $T_{(1:n)}$ as this is the most recent time in the sequence. Note that only the product in \Cref{product_f} needs to be rewritted as $B(dt, dx)$ is, by definition, a function of $N$. 

Our goal is to find a nonnegative function $f$ such that \Cref{product_f} can be expressed as $e^{-N(f)}$.
